"""Launch the Desk alone on 127.0.0.1 (ADR 0034) — a debug entrypoint since ADR 0035.
    ...
    uv run python -m surfaces.desk [--port 8002]
...
The other two: `python -m surfaces.console` and `python -m surfaces.viewer` (ADR 0036).
"""
...
def main(argv: list[str] | None = None) -> None:
    ap = argparse.ArgumentParser(prog="python -m surfaces.desk")
    ...
    uvicorn.run("surfaces.desk.app:app", host=args.host, port=args.port,
                log_level="info")
